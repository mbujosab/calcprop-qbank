...
*** =QuizClozeMulti= — Fichero Moodle tipo cloze

Genera el fichero =.tex= completo para Moodle. Al compilar con =xelatex=, el paquete =moodle= produce el =.xml= importable. El tipo de pregunta se llama /cloze/ en Moodle; dentro del bloque =\begin{cloze}= cada sub-pregunta usa =\begin{multi}=.

La función auxiliar interna =_ClozeMultiBlock= genera el bloque para una sola variante.
