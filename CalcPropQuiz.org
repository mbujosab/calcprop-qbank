...
- Una cadena de texto se escribe como literal Python con =repr()=.
- Un =Supuesto= o =Cuestion= (representado como dict JSON interno) se escribe como llamada al constructor, omitiendo =precond= cuando vale ="True"= y =exp= cuando está vacío.
...
| =⬇ Descargar=  | Muestra en el área de salida un enlace HTML (=data URI=, atributo =download=) para guardar el JSON en el navegador, sin ejecutar JavaScript (que JupyterLab bloquea desde un callback de botón). |
