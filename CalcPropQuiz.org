...
def por_partes(self, instances=1, base_seed=None, verbose=False):
    ...
    ----------
    instances : int
        Número de instancias con semillas distintas (para ejercicios con
        ``setup`` que genera valores aleatorios). Por defecto 1.
    base_seed : int
        Semilla base. La instancia k usa ``numpy.random.seed(base_seed + k * 997)``.
        Si no se pasa, se usa self.seed o 42 como fallback.
    verbose : bool
        Si True, imprime las cuestiones/supuestos rechazados por precondición.
        Por defecto False (silencioso en batch).
    """
    if instances > 1:
        ...
        actual_base = base_seed if base_seed is not None else (self.seed or 42)
        offset = 0
        for inst in range(instances):
            _np.random.seed(actual_base + inst * 997)
            count = 0
            for etiqueta, partes in self._variantes(verbose=verbose):
                count += 1
