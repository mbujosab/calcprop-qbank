...
El módulo =qbank._json= permite guardar y cargar objetos =ProblemaTipo= y =ProblemaVF= en ficheros JSON. Esto facilita compartir bancos de preguntas, usarlos desde el editor visual o persistirlos entre sesiones sin tener que reescribir el código Python en cada ocasión.
...
Los campos =semantica= y =precond= se almacenan como *cadenas de texto*. Al deserializar se evalúan con =eval()= en el espacio de nombres de =calcprop=, lo que permite expresiones como ="v('A') & v('B')"=, los booleanos literales =True= y =False=, o lambdas para semánticas paramétricas (="lambda ns: ns['x'] > 0"=).
...
La función =_slot_to_json= recorre el problema en sentido inverso. Para los objetos =Supuesto= y =Cuestion= recupera el atributo =_json= adjunto durante la carga. Si el objeto fue creado directamente en Python (sin pasar por =problema_from_dict=), no tiene ese atributo y la serialización falla con un mensaje explicativo que orienta al usuario hacia la causa del error:
...
La función inversa, =problema_to_dict=, admite siempre objetos =ProblemaVF= (sus datos son directamente serializables). Para =ProblemaTipo= delega en =_slot_to_json=, que requiere que los componentes lleven el atributo =_json=.
