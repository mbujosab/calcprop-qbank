...
| Función                        | Tipo de problema | Opción rescate | Notas                            |
|--------------------------------+------------------+----------------+----------------------------------|
| =QuizMoodleLastCh=             | =ProblemaTipo=   | Sí ✓           | *Uso general recomendado*        |
| =QuizMoodle=                   | =ProblemaTipo=   | No             | Solo si garantizas ≥1 correcta   |
| =QuizMoodleProfe=              | =ProblemaTipo=   | No             | Revisión del profesor            |
| =QuizMoodleLastChConVariables= | =ProblemaTipo=   | Sí ✓           | Valores numéricos con Jinja2     |
| =QuizMoodleConVariables=       | =ProblemaTipo=   | No             | Ídem sin rescate                 |
| =QuizMoodleProfeConVariables=  | =ProblemaTipo=   | No             | Vista profe con Jinja2           |
...
=QuizMoodleLastCh= es la elección segura salvo que el problema garantice siempre al menos
...
#+begin_src python
QuizMoodleLastCh(nombre, directorio, problema, opc=["", "Las demás opciones son falsas"])
#+end_src
...
QuizMoodleLastCh("Superheroes", "ejemplosManual/", p)
#+end_src
...
Ambos patrones funcionan igual para Moodle: sustituye =AMC= por =QuizMoodleLastCh=
y adapta el nombre del fichero de salida.
...
ProblemaTipo(lista, setup=None)
    → itera (etiqueta, enunciado, cuestiones); .por_partes() para multiparte
...
#+begin_example
QuizMoodleLastCh        (nombre, directorio, problema, opc=["","Las demás..."])  # recomendado
