...
| Función    | Equivalente =AMCblock=                         | Estado         |
|------------+------------------------------------------------+----------------|
| =AMC_VF=   | (no reemplazable, para =ProblemaVF=)           | Vigente        |
| =AMC=      | =AMCblock(..., last_choice=False, cols=1)=      | Deprecado      |
| =AMCProfe= | =AMCblock(..., profe=True)=                    | Deprecado      |
| =AMClastCh=| =AMCblock(..., last_choice=True)=              | Deprecado      |
| =AMCmc=    | =AMCblock(..., cols=ncols)=                    | Deprecado      |
| =AMCmcProfe= | =AMCblock(..., cols=ncols, profe=True)=      | Deprecado      |
| =AMClastChmc= | =AMCblock(..., last_choice=True, cols=ncols)=| Deprecado     |
...
|--------------------+------------------+---------------+-------------------------------|
| =QuizMoodle=       | =ProblemaTipo=   | parámetro     | *Función unificada recomendada* |
| =QuizMoodleProfe=  | =ProblemaTipo=   | No            | Revisión del profesor          |
| =QuizVFMoodle=     | =ProblemaVF=     | —             | Verdadero/Falso                |
| =QuizMoodleLastCh= | =ProblemaTipo=   | Sí (fijo)     | *Deprecado* → =QuizMoodle(..., last_choice=True)= |
...
| Parámetros de =QuizMoodle=             | Cuándo usarla                               |
|----------------------------------------+---------------------------------------------|
| (por defecto)                          | Pregunta estándar.                          |
| =last_choice=True=                     | Añade «Las demás opciones son falsas».      |
| =QuizMoodleProfe(...)=                 | Versión con ítems rechazados visibles.      |
...
Supuesto(enunciado, semantica, precond=True)
Cuestion(enunciado, semantica, precond=True, exp="")
ProblemaTipo(lista, setup=None, export=None)
    → itera (etiqueta, enunciado, cuestiones); .por_partes(instances=1, base_seed=42) para multiparte/instancias
...
# Aliases deprecados (emiten DeprecationWarning, redirigen a AMCblock):
AMC, AMCProfe, AMClastCh, AMCmc, AMCmcProfe, AMClastChmc
#+end_example
...
QuizMoodleLastCh (nombre, directorio, problema, opc=["","Las demás..."])
    # → usa QuizMoodle(..., last_choice=True)
#+end_example
