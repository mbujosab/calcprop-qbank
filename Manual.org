...
| =opc=        | =list=   | =[instrucciones_extra, texto_lastchoice]=. Raramente necesario.            |
...
AMCblock(nombre, etiqueta, enunciado, cuestiones,
         last_choice=False, cols=1, profe=False, opc=["", "Ninguna de las anteriores"])
#+end_src
...
| =opc=         | =[instrucciones_extra, texto_lastchoice]=. Raramente necesario.    |
...
| =opc=         | =[codigo_latex_extra, texto_opcion_rescate]=. Raramente necesario.    |
...
AMC_multipart(nombre, etiqueta, enunciado, partes, opc=[""])
    # para ProblemaTipo multiparte; partes = [(enunciado, cuestiones), ...]
QuizAMCProfe(nombre, directorio, problema, cols=1, opc=["","Ninguna de las anteriores"])
    # alto nivel: vista aplanada profe → escribe nombre_profe.tex
...
QuizMoodle      (nombre, directorio, problema, last_choice=False,
                 opc=["","Las demás opciones son falsas"], instances=1)
    # vista alumno; instances>1 requiere numpy y setup con aleatorios
QuizMoodleProfe (nombre, directorio, problema, opc=["",""])
    # mismas variantes que el alumno, con fracciones de puntuación visibles
QuizVFMoodle    (nombre, directorio, GenVar, num, opc=["",""])
#+end_example
