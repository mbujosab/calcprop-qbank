...
Este es, esencialmente, el problema =numeros.json= que carga en el editor visual el
notebook de demostración =ejemplosManual/Demo.ipynb= (sección «8.2 Editor cargando un
ejemplo con setup»). Puedes lanzarlo en el navegador, sin instalar nada, con Binder:
...
Como el =setup= es aleatorio el iterador no tiene fin y el caso «$a = b$» aparece rara
vez; acota la generación con =itertools.islice= igual que en el resto de ejemplos con
=setup= aleatorio.
